"""Айдой — қазақ тілді AI көмекші, негізгі цикл."""
...
def _normalize_query(text: str) -> str:
    """'Айдой,' префиксін алып тастау."""
    prefixes = ["айдой,", "айдой", "aidos,", "aidos"]
...
def run_text_mode(router: IntentRouter) -> None:
    ...
    while True:
        ...
        try:
            ...
            console.print(f"[bold cyan]Айдой:[/bold cyan] {response}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Айдой — қазақ AI көмекші")
    ...
    def tts_callback(text: str) -> None:
        console.print(f"[bold cyan]Айдой:[/bold cyan] {text}")
        tts.speak(text)
    ...
    _log.info("Айдой іске қосылды, режим=%s, модель=%s",
    ...
    try:
        ...
        console.print("\n[dim]Сау болыңыз![/dim]")
        _log.info("Айдой Ctrl+C арқылы тоқтатылды")
    except Exception as exc:
